...
async function main() {
    ...
    shared_1.logger.info('Stage-One MCP Server - Sync Tool');
    shared_1.logger.info('================================\n');
    if (options.dryRun) {
        shared_1.logger.info('DRY RUN MODE - No changes will be made\n');
    }
    const server = new index_1.StageOneMCPServer();
    // Test connection first
    shared_1.logger.info('Step 1: Testing connection & authenticating...');
    const connected = await server.testConnection();
    if (!connected) {
        shared_1.logger.error('Connection/authentication failed. Aborting sync.');
        process.exit(1);
    }
    // Get current status

...
const supabase_local_1 = require("./supabase_local");
// ============================================
// PLANET WORLD MCP SERVER
// ============================================
class PlanetWorldMCPServer {
    constructor(supabaseUrl, supabaseKey) {
        ...
        this.supabase = new supabase_local_1.PartalSupabaseService(supabaseUrl, supabaseKey);
    }
    // ============================================
    ...
    async syncProducts(options) {
        ...
        try {
            ...
            shared_1.logger.info('🚀 Starting paginated scrape...');
            while (hasMoreProducts && pageNum <= maxPages) {
                if ((productsAdded + productsUpdated) >= limit) {
                    shared_1.logger.info(`🛑 Reached limit of ${limit} products`);
                    break;
                }
                const pageUrl = `${this.config.baseUrl}${this.config.productsUrl}?page=${pageNum}`;
                shared_1.logger.info(`📄 Scraping Page ${pageNum}: ${pageUrl}`);
                try {
                    ...
                    if (productLinks.length === 0) {
                        shared_1.logger.info(`✅ No products found on page ${pageNum}. Reached end of catalog.`);
                        hasMoreProducts = false;
                        break;
                    }
                    shared_1.logger.info(`🔍 Page ${pageNum}: Found ${productLinks.length} products`);
                    // Scrape details for this page
                    const products = await this.scrapeProductDetails(page, productLinks, options === null || options === void 0 ? void 0 : options.dryRun);
                    if (products.length === 0) {
                        shared_1.logger.warn(`⚠️ Page ${pageNum} had links but scraping details returned 0 products.`);
                    ...
                    if (products.length > 0) {
                        ...
                        shared_1.logSync.progress('Planet World', productsAdded + productsUpdated, 0);
                    }
                    pageNum++;
                    ...
                    shared_1.logger.error(`❌ Error scraping page ${pageNum}: ${e.message}`);
    ...
    async capturePricingStock(page, timeoutMs = 5000) {
        try {
        ...
        for (let i = 0; i < productLinks.length; i++) {
            ...
            try {
                ...
                if (networkData.found) {
                    if (dryRun)
                        shared_1.logger.info(`⚡ API Intercept: Price R${networkData.price}, Stock: ${networkData.inStock} (${url})`);
                }
                ...
                const productData = await page.evaluate((productUrl) => {
                    ...
                    const name = ((_b = (_a = document.querySelector('h1')) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.trim()) ||
                        ((_d = (_c = document.querySelector('.product-name')) === null || _c === void 0 ? void 0 : _c.textContent) === null || _d === void 0 ? void 0 : _d.trim()) ||
                        ((_f = (_e = document.querySelector('.product-title')) === null || _e === void 0 ? void 0 : _e.textContent) === null || _f === void 0 ? void 0 : _f.trim()) ||
                        '';
                    // SKU
                    let sku = '';
                    ...
                    for (const selector of brandSelectors) {
                        ...
                        if (brandEl) {
                            brand = ((_g = brandEl.textContent) === null || _g === void 0 ? void 0 : _g.trim()) || '';
                            break;
                        }
                    }
                    ...
                    if (stockEl) {
                        const stockText = ((_h = stockEl.textContent) === null || _h === void 0 ? void 0 : _h.toLowerCase().trim()) || '';
                        if (stockText.includes('in stock'))
                            inStock = true;
                        if (stockText.includes('no stock') || stockText.includes('out of stock'))
                            inStock = false;
                    }
                ...
                if (productData.price === 0) {
                    shared_1.logger.warn(`⚠️  Zero price detected for ${productData.sku} (${url})`);
                    shared_1.logger.warn(`Debug Info: ${productData.debugInfo}`);
                }
    ...
    extractCategory(product) {
        var _a, _b;
        const name = ((_a = product.name) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || '';
        const category = ((_b = product.category) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || '';

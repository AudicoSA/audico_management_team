...
async function main() {
    ...
    shared_1.logger.info('🚀 Planet World MCP Server - Sync Tool');
    shared_1.logger.info('================================\n');
    if (options.dryRun) {
        shared_1.logger.info('⚠️  DRY RUN MODE - No changes will be made\n');
    }
    const server = new index_1.PlanetWorldMCPServer();
    ...
    try {
        ...
        if (status.status === 'error' && status.error_message) {
            shared_1.logger.warn(`⚠️  Previous sync had error: ${status.error_message}`);
            shared_1.logger.info('Proceeding with new sync attempt...');
        }
    }
    catch (error) {
        shared_1.logger.error(`❌ Failed to get status: ${error.message}`);
        shared_1.logger.error(`Stack trace: ${error.stack}`);
        shared_1.logger.info('Attempting to proceed with sync anyway...');
    }
    // Sync products

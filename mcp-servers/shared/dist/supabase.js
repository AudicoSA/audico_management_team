...
class SupabaseService {
    ...
    async getSupplierByName(name) {
        ...
        if (error) {
            ...
            logger_1.logger.error(`Error getting supplier ${name}: ${error.message}`);
            return null;
        }
        return data;
    }
    ...
    async createSyncSession(supplierId, name) {
        // Determine status based on running state (mapped to 'running')
        // We use mcp_sync_sessions table based on migration 011
        const { data, error } = await this.client.from('mcp_sync_sessions').insert({
            triggered_by: name, // storing session name in triggered_by
            status: 'running',
    ...
    async logCrash(params) {
        try {
            ...
            logger_1.logger.error(`💥 CRASH: ${params.supplierName} - ${params.errorType}: ${params.errorMessage}`);
            ...
            if (error) {
                ...
                logger_1.logger.error(`Failed to log crash to database: ${error.message}`);
            }
        }
        catch (err) {
            // Defensive: never let crash logging itself crash
            logger_1.logger.error(`Crash logging failed: ${err}`);
        }
    }
}

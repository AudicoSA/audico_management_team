...
function parseZAR(raw) {
    ...
    if (cleaned.includes('.') && cleaned.includes(',')) {
        ...
        if (/,(\d{2})$/.test(cleaned))
            ...
            cleaned = cleaned.replace(/,/g, ''); // thousands
    }
    const n = Number.parseFloat(cleaned);
    return Number.isFinite(n) ? n : 0;
}
export class HomemationMCPServer {
    supabase;
    supplier = null;
    config = {
    ...
    constructor(supabaseUrl, supabaseKey) {
        this.supabase = new SupabaseService(supabaseUrl || process.env.NEXT_PUBLIC_SUPABASE_URL, supabaseKey || process.env.SUPABASE_SERVICE_KEY);
    }
    ensureNoTrailSlash(url) {
    ...
    async syncProducts(options) {
        ...
        try {
            ...
            for (let i = 0; i < this.config.categoryUrls.length; i++) {
                ...
                collected.push(...rows);
                if (i % 50 === 0)
                    logSync.progress('Homemation', i, this.config.categoryUrls.length);
    ...
    async getSupplierInfo() {
        const supplier = await this.supabase.getSupplierByName('Homemation');
        if (!supplier)
            throw new Error('Homemation supplier not found');
        return supplier;
    }
}

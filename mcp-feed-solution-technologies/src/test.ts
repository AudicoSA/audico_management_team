...
import { SolutionTechnologiesMCPServer } from './index';
import { logger } from '@audico/shared';

async function main() {
  logger.info('🧪 Solution Technologies MCP Server - Test Suite');
  logger.info('================================\n');

  const server = new SolutionTechnologiesMCPServer();
  ...
  logger.info('\nNext steps:');
  logger.info('1. Ensure Solution Technologies supplier exists in Supabase');

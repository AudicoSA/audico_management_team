  ...
  const OwnerCheckbox = ({ order, field, label, color }: any) => {
    ...
    return (
      <td
        ...
        className={`text-center py-1 cursor-pointer hover:opacity-75 transition-opacity border-r border-gray-200 ${
          checked ? color : 'bg-gray-50'
        }`}
        title={`Toggle ${label}`}
      >
        {checked && '✓'}
              ...
              <tbody className="divide-y divide-gray-200">
                {orders
                  ...
                  <tr key={order.order_no} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-3 py-1 text-center border-r border-gray-200">
                      <div
                        ...
                        className={`cursor-pointer hover:opacity-75 transition-opacity ${
                          order.order_paid ? 'text-green-600' : 'text-gray-300'
                        }`}
                      >
                        {order.order_paid ? '✓' : '○'}
                      </div>
                    ...
                    <td className={`px-3 py-1 text-right font-semibold border-r border-gray-200 ${

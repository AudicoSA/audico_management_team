...
export class ClaudeConversationHandler {
  ...
  async chat(customerMessage: string): Promise<ChatResponse> {
    ...
    try {
      ...
      const errorDetails = error?.message || error?.name || "Unknown error";
      return {
        message: `I apologize, I encountered an issue: ${errorDetails}. Could you please rephrase your request?`,
        products: [],
      };
    }
  }
  ...
  private async handleAddToQuote(input: Record<string, unknown>): Promise<ToolResult> {
    ...
    try {
      ...
      console.error(`[ClaudeHandler] Failed to add product to quote:`, error);
      return {
        success: false,

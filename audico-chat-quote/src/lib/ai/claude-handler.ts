  ...
  private async loadConversationHistoryFromDB(): Promise<void> {
    try {
      ...
      validatedHistory.push(msg);
        ...
        // SECOND PASS: Handle Unanswered Tool Uses (The "Line 2 Crash" Fix)
        // If Assistant has 'tool_use' but next message is NOT 'tool_result', we must strip the tool_use.
      ...
      if (msg.role === 'assistant' && Array.isArray(msg.content)) {
        ...
        if (hasToolUse) {
          ...
          if (!isFollowedByResult) {
            ...
            // Filter out tool_use blocks, keep text
            ...
            if (textOnly.length === 0) {
              ...
              continue; // Drop entire message if it was only a tool call
            }

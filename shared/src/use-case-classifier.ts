...
const BRAND_USE_CASE: Record<string, UseCase> = {
  // Home audio brands
  'denon': 'Home',
  'marantz': 'Home',
  'onkyo': 'Home',
...
const CATEGORY_USE_CASE: Record<string, UseCase> = {
  'car audio': 'car_audio',
 ...
 */
export function classifyUseCase(input: ClassifierInput): UseCase {
  const searchText = [
    input.productName,
    input.categoryName,
    input.description
  ...
  let maxScore = 0;
  let result: UseCase = 'Home'; // Default to Home if no matches

  for (const [useCase, score] of Object.entries(scores)) {
    if (score > maxScore) {
      maxScore = score;
      result = useCase as UseCase;
    }
  }
 ...
 */
export function shouldExcludeFromConsultation(useCase: UseCase): boolean {
  return useCase === 'car_audio';
}

export default { classifyUseCase, shouldExcludeFromConsultation };

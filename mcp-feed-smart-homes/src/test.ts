...
import { SmartHomesMCPServer } from './index';
import { logger } from '@audico/shared';

async function main() {
  logger.info('🧪 Nology MCP Server - Test Suite');
  logger.info('================================\n');

  const server = new SmartHomesMCPServer();
  ...
  logger.info('\nNext steps:');
  logger.info('1. Run: npm run sync -- --limit=10');
  logger.info('2. Check Supabase products table');
  logger.info('3. Run full sync: npm run sync');
}
